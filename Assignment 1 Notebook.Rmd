...
5) Optimal budget-setting to minimize project completion delays
  ...
  (c) 
    1. Have multiple teams with comparable capacities and characteristics (e.g. same amount of people with a comparable background and seniority on each team) simultaneously assigned (randomly) to some of the villages in a large area.         2. Assign all of them the same project and original completion date, for example increasing the percentage of school age children enrolled in some elementary school by 25% in 2 years.
    3. Analyze initial baselines for the variable which is the goal of the project (school enrollment in this example); make sure that they are relatively similar across villages.
    4. Randomly assign different levels of funding to each of the teams.
    5. Wait until each project is completed, regardless of the original completion date.
    6. Compare delay times between teams with more or less funding.  
